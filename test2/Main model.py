...
axes[0].set_title("Top 5 Factors Correlated with Carlos Alcara\'s momentum\n(NJ is Nicolas Jarry,CA is Carlos Alcaraz)")
axes[0].set_xlabel('Correlation')

# 调整y轴标签位置
axes[0].set_yticks(range(len(p1_top5_df.index)))
axes[0].set_yticklabels(["CA's momentum","CA's winning innings","Number of games played","Score gap","NJ's winning innings"])

# 绘制p2_momentum相关性最强的5个因素
p2_top5_df.plot(kind='barh', ax=axes[1], legend=False, color='lightcoral')
axes[1].set_title('Top 5 Factors Correlated with Nicolas Jarry\'s momentum\n(NJ is Nicolas Jarry,CA is Carlos Alcaraz)')
axes[1].set_xlabel('Correlation')

# 调整y轴标签位置
axes[1].set_yticks(range(len(p2_top5_df.index)))
axes[1].set_yticklabels(["NJ's momentum","NJ's winning innings","Number of games played","CA's winning innings","NJ's score"])
...
plt.text(220,100,f"Jarry's P value is:{p_value2}",fontsize = 7)
plt.plot(data['score_gap'][:300], label = 'score gap')
plt.plot(data['p1_momentum'][:300], label = 'Carlos Alcaraz')
plt.plot(data['p2_momentum'][:300], label = 'Nicolas Jarry')
plt.legend()
plt.grid(True)
plt.show()
